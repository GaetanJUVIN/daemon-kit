...
module DaemonKit
  ...
  class Initializer
    ...
    def self.shutdown( clean = false, do_exit = false )
      ...
      DaemonKit.logger.warn("DaemonKit::Initializer | Shutting down #{DaemonKit.configuration.daemon_name}")
    ...
    def after_daemonize
      ...
      DaemonKit.logger.info("#{self.class} | DaemonKit now running #{DaemonKit.configuration.daemon_name}" )
      ...
      if DaemonKit.configuration.user || DaemonKit.configuration.group
        ...
        uid = Process.uid
        gid = Process.gid
        DaemonKit.logger.info("#{self.class} | DaemonKit dropped privileges to: #{euid} (EUID), #{egid} (EGID), #{uid} (UID), #{gid} (GID)"  )
      end
    end
    ...
    def initialize_logger
      ...
      DaemonKit.logger.info("#{self.class} | DaemonKit (#{DaemonKit.version}) booting in #{DAEMON_ENV} mode")

      configuration.trap("USR1") {
        DaemonKit.logger.level = DaemonKit.logger.debug? ? :info : :debug
        DaemonKit.logger.info("#{self.class} | Log level changed to #{DaemonKit.logger.debug? ? 'DEBUG' : 'INFO' }")
      }
      configuration.trap("USR2") {
        DaemonKit.logger.level = :debug
        DaemonKit.logger.info("#{self.class} | Log level changed to DEBUG")
      }
    ...
    def configure_safely
      DaemonKit.logger.info("#{self.class} | Configuring safely for exception handling")
  ...
  class Configuration
    ...
    def trap( signal, proc = nil, &block )
      ...
      unless Signal.list.include?( signal )
        DaemonKit.logger.warn("#{self.class} | Trapping #{signal} signals not supported on this platform")
        return
      end
    ...
    def run_traps( signal )
      DaemonKit.logger.info("#{self.class} | Running signal traps for #{signal}")
    ...
    def set_trap( signal )
      DaemonKit.logger.info("#{self.class} | Setting up trap for #{signal}")
      @signal_traps[ signal ] = []
    ...
    def error( msg )
      ...
      if DaemonKit.logger
        DaemonKit.logger.error("#{self.class} | #{msg}")
      else
        STDERR.puts msg
      end
    end
